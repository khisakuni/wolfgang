...
class Staff extends Component {
    ...
    return React.Children.map(
      ...
      (measureComponent, i) => {
        const width =
          ...
          React.Children.count(this.measureComponents());
        const height = 100;
        const x = i * width;
        ...
        let key;
        if (measure) {
          const notes = _.values(this.props.notes).filter(
            note => note.measureId === measure.id
          );
          key = notes.reduce(
            (acc, note) => (acc += note.value + note.duration),
            ""
          );
        }
  ...
  render() {
    return <div>{this.renderMeasures()}</div>;
  }
}
